...
# Each letter replaced by a letter with a 'key' times itself positions down the alphabet.
...
def encrypt(text, key):
    ...
    for c in text:
        if c == ' ':
            ...
            new_pos = ((ord(c) - 65) * key) % 26
            encrypted_txt += chr(new_pos+65)
        else:
            new_pos = ((ord(c) - 97) * key) % 26
...
def decrypt(text, key):
    ...
    for c in text:
        if c == ' ':
            ...
            new_pos = ((ord(c) - 65) * minv) % 26
            decrypted_txt += chr(new_pos + 65)
        else:
            new_pos = ((ord(c) - 97) * minv) % 26
